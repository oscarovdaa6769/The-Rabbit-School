...
<body class="bg-white text-brand-brown">
  ...
  <section class="relative overflow-hidden">
    ...
    <div class="anim-fade-up max-w-4xl mx-auto text-center px-6 py-24 md:py-32">
      ...
      <div class="flex flex-col sm:flex-row items-center justify-center gap-4">
        <a href="#shop" class="click-fx bg-brand-yellow text-brand-brown font-semibold px-8 py-3 rounded-full hover:bg-white transition"> <?php echo esc_html($btn_1); ?> </a>
      </div>
    </div>
  </section>
  ...
  <section class="max-w-7xl mx-auto px-6 py-16">
    <div class="grid grid-cols-2 md:grid-cols-3 gap-8 md:gap-6 text-center">

      <div class="anim-slide-left anim-delay-1 flex flex-col items-center gap-3">
        <div class="w-14 h-14 rounded-full bg-[#DDB0D1] flex items-center justify-center text-2xl">
          <img src="<?php echo esc_url(get_theme_file_uri('assets/icons/express-delivery(2).png'));?>"
                alt="Delivery Icon"
                loading="lazy"
      ...
      <div class="anim-fade-up anim-delay-2 flex flex-col items-center gap-3">
        <div class="w-14 h-14 rounded-full bg-[#8BAEA7] flex items-center justify-center text-2xl">
          <img src="<?php echo esc_url(get_theme_file_uri('assets/icons/graduation-hat.png'));?>"
                alt="Delivery Icon"
                loading="lazy"
                class="w-8 h-8 object-contain filter drop-shadow-[0_2px_4px_rgba(0,0,0,0.06)]" />
        </div>
      ...
      <div class="anim-slide-right anim-delay-3 flex flex-col items-center gap-3">
        <div class="w-14 h-14 rounded-full bg-[#FED45F] flex items-center justify-center text-2xl">
          <img src="<?php echo esc_url(get_theme_file_uri('assets/icons/chat.png'));?>"
                alt="Delivery Icon"
                loading="lazy"
                class="w-8 h-8 object-contain filter drop-shadow-[0_2px_4px_rgba(0,0,0,0.06)]" />
        </div>
  ...
  <section id="shop" class="bg-brand-cream py-20">
    <div class="max-w-7xl mx-auto px-6">
      ...
      <div id="filterStatus" class="hidden items-center gap-3 mb-6">
        ...
        <button onclick="clearFilter()" class="click-fx text-xs font-semibold text-brand-brown bg-white border border-brand-brown/20 px-3 py-1.5 rounded-full hover:bg-brand-brown hover:text-white transition">Clear filter ✕</button>
      </div>
...
?>
  ...
  <div class="product-card click-fx bg-white rounded-xl overflow-hidden border border-brand-brown/10 flex flex-col" data-category="<?php echo esc_attr( $category_slug ); ?>">
    ...
    <div class="p-3 flex flex-col flex-1">
      ...
      <div class="flex items-center justify-between pt-2 border-t border-brand-brown/5">
        ...
        <button onclick="openOrderModal('<?php echo esc_js( get_the_title() ); ?>', '<?php echo esc_js( $price ); ?>')" class="click-fx bg-brand-yellow text-brand-brown text-xs font-semibold px-3 py-1.5 rounded-full whitespace-nowrap"><?php echo esc_html( $detail ); ?></button>
      </div>
    </div>
  </div>
...
?>
  ...
  <div id="orderModal" class="hidden fixed inset-0 z-50 items-center justify-center p-4">
    ...
    <div class="relative bg-white rounded-3xl w-full max-w-md p-7 md:p-9 shadow-2xl">
      ...
      <p class="uppercase tracking-widest text-brand-orange text-xs font-semibold mb-2"><?php echo (function_exists('pll_current_language') && pll_current_language() === 'km') ? 'ចាប់អារម្មណ៍លើវត្ថុនេះ?' : 'Interested in this item?'; ?></p>
      ...
      <p class="text-xs font-semibold uppercase tracking-wide text-text-muted mb-3"><?php echo (function_exists('pll_current_language') && pll_current_language() === 'km') ? 'ទាក់ទងមកយើងខ្ញុំដើម្បីបញ្ជាទិញ' : 'Contact us to order'; ?></p>
      <div class="grid grid-cols-1 gap-2.5">

        <!-- Email / Gmail -->
        <button type="button" onclick="submitOrder('gmail')" class="click-fx group flex items-center gap-3 w-full bg-white rounded-xl px-4 py-3 border border-brand-brown/10 shadow-sm hover:shadow-md hover:-translate-y-0.5 active:translate-y-0 active:shadow-sm transition-all duration-200">
          ...
          <span class="text-sm font-semibold text-brand-brown"><?php echo (function_exists('pll_current_language') && pll_current_language() === 'km') ? 'អ៊ីមែល / ជីមែល' : 'Email / Gmail'; ?></span>
          <span class="ml-auto text-text-muted/40 group-hover:text-brand-brown group-hover:translate-x-1 transition-all text-xs">➔</span>
        </button>

        <!-- Facebook -->
        <button type="button" onclick="submitOrder('facebook')" class="click-fx group flex items-center gap-3 w-full bg-white rounded-xl px-4 py-3 border border-brand-brown/10 shadow-sm hover:shadow-md hover:-translate-y-0.5 active:translate-y-0 active:shadow-sm transition-all duration-200">
          ...
          <span class="text-sm font-semibold text-brand-brown"><?php echo (function_exists('pll_current_language') && pll_current_language() === 'km') ? 'ទំព័រហ្វេសប៊ុក' : 'Facebook Page'; ?></span>
          <span class="ml-auto text-text-muted/40 group-hover:text-brand-brown group-hover:translate-x-1 transition-all text-xs">➔</span>
        </button>

        <!-- Instagram -->
        <button type="button" onclick="submitOrder('instagram')" class="click-fx group flex items-center gap-3 w-full bg-white rounded-xl px-4 py-3 border border-brand-brown/10 shadow-sm hover:shadow-md hover:-translate-y-0.5 active:translate-y-0 active:shadow-sm transition-all duration-200">
          ...
          <span class="text-sm font-semibold text-brand-brown"><?php echo (function_exists('pll_current_language') && pll_current_language() === 'km') ? 'សារផ្ទាល់តាមអ៊ីនស្តាក្រាម' : 'Instagram Direct'; ?></span>
          <span class="ml-auto text-text-muted/40 group-hover:text-brand-brown group-hover:translate-x-1 transition-all text-xs">➔</span>
        </button>
