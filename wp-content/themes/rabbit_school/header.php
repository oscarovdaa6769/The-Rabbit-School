...
<body <?php body_class( 'bg-brand-cream text-brand-brown font-sans min-h-screen flex flex-col' ); ?>> 
  ...
  <header class="bg-white sticky top-0 z-[1000] border-b border-gray-100 px-4 sm:px-6 lg:px-8">
    <div class="max-w-7xl mx-auto flex items-center justify-between h-24">
      ...
      <!-- Logo -->
      <a href="<?php echo esc_url( home_url( '/' ) ); ?>" class="flex items-center gap-3 group shrink-0 no-underline">
      ...
      <nav class="hidden lg:flex items-center font-semibold text-sm">
        <?php wp_nav_menu( array(
          ...
          'menu_class'     => 'flex items-center gap-[10px] font-semibold text-sm 
                               ...
                               [&_a:hover]:text-white [&_a:hover]:bg-brand-brown 
                               [&_.current-menu-item_a]:bg-brand-brown [&_.current-menu-item_a]:text-white [&_.current-menu-item_a]:border-white [&_.current-menu-item_a]:ring-brand-brown'
        ) ); ?>
      </nav>
      ...
      <div class="flex items-center gap-[10px] sm:gap-4">
        ...
        <div class="hidden sm:inline-block text-left group relative">
          <button type="button" class="text-sm font-semibold text-brand-brown hover:bg-brand-brown hover:text-white flex items-center gap-2 border border-brand-brown px-[20px] py-[10px] rounded-[8px] transition-colors duration-200 focus:outline-none cursor-pointer">
            <span class="icon-[solar--global-bold] w-5 h-5 flex-shrink-0" aria-hidden="true"></span>
            ...
            <span class="icon-[solar--alt-arrow-down-line-duotone] w-5 h-5 transition-transform duration-200 group-hover:rotate-180" aria-hidden="true"></span>
          </button>
          ...
          <div class="absolute right-0 pt-2 w-40 hidden group-hover:block z-50 transition-all duration-200">
            <div class="bg-brand-cream border border-brand-brown/20 rounded-[12px] shadow-lg py-2
                        ...
                        [&_a]:block [&_a]:px-4 [&_a]:py-2 [&_a]:text-sm [&_a]:text-brand-brown [&_a]:font-semibold [&_a]:transition-colors [&_a]:duration-150
                        [&_a]:hover:bg-brand-brown [&_a]:hover:text-white [&_a]:no-underline">
              <?php 
              wp_nav_menu( array(
        ...
        if ( isset( $locations['donate'] ) && $menu = wp_get_nav_menu_object( $locations['donate'] ) ) {
          ...
          if ( ! empty( $menu_items ) ) {
            ...
            ?>
            <a href="<?php echo esc_url( $donate->url ); ?>" class="group bg-brand-yellow hover:bg-brand-orange text-brand-brown hover:text-white font-bold text-sm px-[24px] py-[12px] rounded-[8px] shadow-md hover:shadow-xl hover:scale-105 active:scale-95 transition-all duration-300 flex items-center gap-2 tracking-wider uppercase no-underline">
        ...
        <!-- Mobile Menu Toggle -->
        <button id="mobile-menu-btn" class="lg:hidden text-brand-brown p-2 focus:outline-none flex items-center justify-center cursor-pointer" aria-label="Toggle Navigation Menu" aria-expanded="false">
          <span id="hamburger-icon-slot" class="block">
            <span class="icon-[solar--hamburger-menu-linear] w-6 h-6" aria-hidden="true"></span>
          </span>
          <span id="close-icon-slot" class="hidden">
            <span class="icon-[solar--close-circle-linear] w-6 h-6" aria-hidden="true"></span>
          </span>
        </button>
      </div>
    </div>
      ...
      <nav class="flex flex-col">
        <?php wp_nav_menu( array(
          ...
          'menu_class'     => 'flex flex-col gap-1 font-semibold text-sm
                               [&_a]:block [&_a]:px-4 [&_a]:py-3 [&_a]:rounded-lg [&_a]:no-underline
                               [&_a:hover]:bg-brand-brown [&_a:hover]:text-white
                               [&_.current-menu-item_a]:bg-brand-brown [&_.current-menu-item_a]:text-white'
        ) ); ?>
      </nav>
          ...
          if ( function_exists( 'pll_the_languages' ) ) :
            ...
            foreach ( $languages as $lang ) :
              ...
              <a href="<?php echo esc_url( $lang['url'] ); ?>"
                 class="px-3 py-1.5 text-xs font-bold rounded-md no-underline transition-colors <?php echo $lang['current_lang'] ? 'bg-brand-brown text-white' : 'border border-gray-200 text-brand-brown hover:bg-gray-50'; ?>">
